...
/*   Updated: 2021/06/10 15:33:55 by hyseo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

char	*check_print_xx(char *result, t_flag t, int len, int str_len)
{
	char	*re;
	int		i;
	char	zero;
...
char	*set_print_xx(char *result, int str_len)
{
	char	*re;
	int		i;
	int		j;
...
int		print_xx(unsigned int result, t_flag t, int re_count)
{
	...
	i = 0;
	m_free = ft_itoxx(result);
	str_len = ft_strlen(m_free);
	set_pre(result, &t, str_len);
	len = (t.width >= t.precision ? t.width : t.precision);
	print_result = check_print_xx(
		set_print_xx(m_free, t.precision), t, len, t.precision);
